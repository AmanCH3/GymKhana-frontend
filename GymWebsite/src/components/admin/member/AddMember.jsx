...
const AddMember = ({ modalOpen, handleClose, member }) => {
  const [formData, setFormData] = useState({
    ...
    membership_type: "", // Store the selected membership_type_id
  });
  const [loading, setLoading] = useState(false);

  // Example array of membership types
  ...
  useEffect(() => {
    if (member) {
      setFormData({
        ...
        membership_type: member.membership_type.membership_type_id, // Store the ID
      });
    }
  }, [member]);
  ...
  const handleSelectMembershipType = (typeId) => {
    setFormData((prev) => ({
      ...prev,
      membership_type: typeId, // Store the selected membership_type_id
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      membership_type_id: formData.membership_type, // Send the ID directly
    };
    try {
      let response;
      if (member) {
